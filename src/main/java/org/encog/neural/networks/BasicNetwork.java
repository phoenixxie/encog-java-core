...
public class BasicNetwork extends BasicML implements ContainsFlat, MLContext,
	...
	public void decodeFromArray(double[] encoded, double[][] bias) {
		...
		final double[][] biases = this.structure.getFlat().getBiases();
		if (biases.length != bias.length) {
			throw new NeuralNetworkError(
					"Size mismatch, bias array should be of length "
							+ biases.length);
		}

		for (int i = 0; i < bias.length; ++i) {
			if (biases[i].length != bias[i].length) {
				throw new NeuralNetworkError("Size mismatch, bias[" + i
						+ "] array should be of length " + biases[i].length);
			}
			EngineArray.arrayCopy(bias[i], biases[i]);
		}
	}
